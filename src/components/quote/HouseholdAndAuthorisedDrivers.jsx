...
function DriverEditForm({ driver, onSave, onCancel, type }) {
  const [form, setForm] = useState(driver || {
    nameDisplayedOnCard: '', nricFin: '', dobDay: '', dobMonth: '', dobYear: '',
    gender: '', licenceYears: '', claims: '', atFaultTimes: '', com: '', type,
  });
...
function DriverSection({ title, description, drivers, onAdd, onRemove, type }) {
  ...
  const badgeClass = type === 'household' ? 'text-cyan' : 'text-emerald-600';
  const badgeText = type === 'household' ? 'Household Member' : 'Authorised Driver';

  return (
    <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-4">
      <div>
        <p className="font-montserrat font-bold text-sm text-carbon">{title}</p>
        <p className="text-xs text-muted-foreground font-montserrat mt-1">{description}</p>
      </div>
      ...
      <div className="space-y-3">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="font-montserrat font-bold text-sm text-carbon">{d.nameDisplayedOnCard}</p>
                    <span className={`text-xs font-montserrat font-semibold ${badgeClass}`}>{badgeText}</span>
                  </div>
                  <p className="text-xs text-muted-foreground font-montserrat mt-1">DOB: {d.dobDay}/{d.dobMonth}/{d.dobYear} · {d.gender}</p>
                </div>
                <button
                  type="button"
                  onClick={() => onRemove(i)}
        ...
        <AnimatePresence>
          {showForm ? (
            ...
            >
              <Plus className="w-4 h-4" /> Add {type === 'household' ? 'Household Member' : 'Authorised Driver'}
            </motion.button>
          )}
        </AnimatePresence>
      </div>
...
export default function HouseholdAndAuthorisedDrivers({ formData, onChange }) {
  const householdMembers = formData.householdMembers || [];
  const authorisedDrivers = formData.authorisedDrivers || [];
  const totalDrivers = householdMembers.length + authorisedDrivers.length;
  const householdCount = householdMembers.length;
  const authorisedCount = authorisedDrivers.length;

  return (
    <div className="space-y-4">
      {/* Summary counter */}
      <div className="bg-grey100 rounded-lg p-3">
        <p className="font-montserrat font-bold text-sm text-carbon">
          Drivers listed: {householdCount} household + {authorisedCount} authorised
        </p>
      </div>
      ...
      <DriverSection
        ...
        description="People who live at the same address as you and may drive your car."
        drivers={householdMembers}
        onAdd={(updated) => onChange('householdMembers', updated)}
        onRemove={(idx) => onChange('householdMembers', householdMembers.filter((_, i) => i !== idx))}
        type="household"
      />
      ...
      {/* Authorised Drivers Section */}
      <DriverSection
        title="Authorised Drivers"
        description="People you have specifically authorised to drive your car who do not live with you."
        drivers={authorisedDrivers}
        onAdd={(updated) => onChange('authorisedDrivers', updated)}
        onRemove={(idx) => onChange('authorisedDrivers', authorisedDrivers.filter((_, i) => i !== idx))}
        type="authorised"
      />
    </div>
  );
